...
type publishOptions struct {
	...
	// RequireSignedMetadata, if true, causes the publish job to fail unless the
	// staged metadata.json has a valid signature verifiable against SigningKMSKey.
	// It is plumbed through to the 'gcb publish' step.
...
func (o *publishOptions) AddFlags(fs *flag.FlagSet, markRequired func(string)) {
	...
	fs.BoolVar(&o.RequireSignedMetadata, "require-signed-metadata", false, "Require the staged metadata.json to have a valid signature verifiable against the signing KMS key before publishing.")
...
func (o *publishOptions) print() {
	...
	log.Printf("  RequireSignedMetadata: %t", o.RequireSignedMetadata)
...
func runPublish(rootOpts *rootOptions, o *publishOptions) error {
	...
	build.Substitutions["_REQUIRE_SIGNED_METADATA"] = fmt.Sprintf("%v", o.RequireSignedMetadata)
